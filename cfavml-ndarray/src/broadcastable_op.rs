    ...
    for<'buf> &'buf mut [MaybeUninit<A>]: WriteOnlyBuffer<Item = A>,
{
    if lhs.ndim() == rhs.ndim() && lhs.shape() == rhs.shape() {
        ...
        let lhs_read_view =
            core::slice::from_raw_parts(lhs_mut_view.as_ptr(), lhs_mut_view.len());
        let lhs_mut_view =
            core::mem::transmute::<&mut [A], &mut [MaybeUninit<A>]>(lhs_mut_view);
        op(lhs_read_view, rhs_view, lhs_mut_view);

        out
    } else {
        let (lhs_view, rhs_view) =
            crate::broadcast_shim::broadcast_mut_with(&lhs, &rhs).unwrap();
        if lhs_view.shape() == lhs.shape() {
            ...
            let mut lhs_buf =
                crate::utils::get_mut_contiguous_slice_or_reallocate(&mut out);
            ...
            match lhs_buf {
                ...
                ArrayMutBackingBuffer::Owned(arr) => arr
                    .into_dimensionality::<<D as DimMax<E>>::Output>()
                    .unwrap(),
            }
            ...
            // CFAVML's mem loaders really should be able to support doing this without reallocating
            // but is not currently implemented and likely needs to adjustment in ndarray to correctly
            // be able to select multiple pointers and load them.
            match (lhs_buf, rhs_buf) {
                ...
                (
                    ArrayBackingBuffer::Borrowed(lhs_view),
                    ArrayBackingBuffer::Borrowed(rhs_view),
                ) => {
                    let mut output =
                        Vec::<MaybeUninit<A>>::with_capacity(lhs_view.len());
                    output.set_len(lhs_view.len());

                    op(lhs_view, rhs_view, &mut output);

                    let initialized =
                        core::mem::transmute::<Vec<MaybeUninit<A>>, Vec<A>>(output);
                ...
                (ArrayBackingBuffer::Owned(mut lhs_arr), rhs_buf) => {
                    ...
                    let lhs_read_view = core::slice::from_raw_parts(
                        lhs_mut_view.as_ptr(),
                        lhs_mut_view.len(),
                    );
                    let lhs_mut_view = core::mem::transmute::<
                        &mut [A],
                        &mut [MaybeUninit<A>],
                    >(lhs_mut_view);
                    op(lhs_read_view, rhs_view, lhs_mut_view);
                ...
                (lhs_buf, ArrayBackingBuffer::Owned(mut rhs_arr)) => {
                    ...
                    let rhs_read_view = core::slice::from_raw_parts(
                        rhs_mut_view.as_ptr(),
                        rhs_mut_view.len(),
                    );
                    let rhs_mut_view = core::mem::transmute::<
                        &mut [A],
                        &mut [MaybeUninit<A>],
                    >(rhs_mut_view);
...
mod tests {
    ...
    fn test_same_shape_op() {
        ...
        let expected = &a + &b;
        let res = unsafe {
            apply_op_with_broadcasting(a, b.view(), |a, b, w| {
                cfavml::add_vertical(a, b, w)
            })
        };
    ...
    #[test]
    fn test_broadcast_stretch_both_axis() {
        let a = Array::random((1, 3, 3, 1), Uniform::new(0.0, 10.0));
        let b = Array::random((3, 1, 1, 3), Uniform::new(0.0, 10.0));
    ...
    #[test]
    fn test_stretch_rhs_with_view() {
        let a = Array::random((3, 3, 1), Uniform::new(0.0, 10.0));
        let b = ArrayView::from_shape((1, 1, 3), &[1.0, 2.0, 3.0]).unwrap();
    ...
    fn test_broadcast_from_arr_slices() {
        ...
        let res = unsafe {
            apply_op_with_broadcasting(slice_a.to_owned(), slice_b.view(), |a, b, w| {
                cfavml::add_vertical(a, b, w)
            })
        };
        assert_eq!(res.shape(), expected.shape());
    ...
    fn test_broadcast_value() {
        ...
        let expected = &a + 4.0;
        let res = unsafe {
            apply_op_with_broadcast_value(a, 4.0, |a, b, w| {
                cfavml::add_vertical(a, b, w)
            })
        };
        assert_eq!(res.shape(), expected.shape());
