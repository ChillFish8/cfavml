...
mod tests {
    ...
    fn test_mut_owned_array_contiguous() {
        ...
        assert!(
            matches!(buffer, ArrayMutBackingBuffer::Borrowed(_)),
            "Buffer should be using borrowed"
        );
    }
    ...
    fn test_owned_array_contiguous() {
        ...
        assert!(
            matches!(buffer, ArrayBackingBuffer::Borrowed(_)),
            "Buffer should be using borrowed"
        );
    }
    ...
    fn test_broadcast_array_non_contiguous() {
        ...
        let basic_view = arr.broadcast((4, 4)).expect("Broadcast shape");
        let buffer = get_contiguous_slice_or_reallocate(&basic_view);
        assert!(
            matches!(buffer, ArrayBackingBuffer::Owned(_)),
            "Buffer should be using borrowed"
        );
    }

    #[test]
    fn test_arc_array_contiguous() {
        // Arc is COW.
