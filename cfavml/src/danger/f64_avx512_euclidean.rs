...
#[inline]
/// Computes the Euclidean distance of two `[f64; DIMS]` vectors.
///
/// # Safety
///
...
pub unsafe fn f64_xconst_avx512_fma_euclidean<const DIMS: usize>(
    x: &[f64],
    y: &[f64],
) -> f64 {
...
/// Values within the vector should also be finite, although it is not
/// going to crash the program, it is going to produce insane numbers.
pub unsafe fn f64_xany_avx512_fma_euclidean(x: &[f64], y: &[f64]) -> f64 {
    debug_assert_eq!(x.len(), y.len());
    ...
    while i < len {
        ...
        let diff = _mm512_sub_pd(x, y);
        acc1 = _mm512_fmadd_pd(diff, diff, acc1);
...
unsafe fn execute_f64_x64_fma_block_euclidean(
    ...
    let diff2 = _mm512_sub_pd(x2, y2);
    let diff3 = _mm512_sub_pd(x3, y3);
    let diff4 = _mm512_sub_pd(x4, y4);
    let diff5 = _mm512_sub_pd(x5, y5);
    let diff6 = _mm512_sub_pd(x6, y6);
    let diff7 = _mm512_sub_pd(x7, y7);
    let diff8 = _mm512_sub_pd(x8, y8);

    *acc1 = _mm512_fmadd_pd(diff1, diff1, *acc1);
    *acc2 = _mm512_fmadd_pd(diff2, diff2, *acc2);
    *acc3 = _mm512_fmadd_pd(diff3, diff3, *acc3);
    *acc4 = _mm512_fmadd_pd(diff4, diff4, *acc4);
    *acc5 = _mm512_fmadd_pd(diff5, diff5, *acc5);
    *acc6 = _mm512_fmadd_pd(diff6, diff6, *acc6);
    *acc7 = _mm512_fmadd_pd(diff7, diff7, *acc7);
    *acc8 = _mm512_fmadd_pd(diff8, diff8, *acc8);
}
...
mod tests {
    ...
    fn test_xconst_fma_euclidean() {
        ...
        let dist = unsafe { f64_xconst_avx512_fma_euclidean::<1024>(&x, &y) };
    ...
    fn test_xany_fma_euclidean() {
        ...
        let dist = unsafe { f64_xany_avx512_fma_euclidean(&x, &y) };
        assert_is_close(dist as f32, simple_euclidean(&x, &y) as f32);
    }
}
