...
/// This method assumes AVX2 instructions are available, if this method is executed
/// on non-AVX2 enabled systems, it will lead to an `ILLEGAL_INSTRUCTION` error.
pub unsafe fn f64_xany_avx2_nofma_min_vertical(matrix: &[&[f64]]) -> Vec<f64> {
    ...
    if offset_from != 0 {
        ...
        while i < (len - tail) {
            ...
            copy_avx2_pd_register_to(min_values_ptr.add(i), acc);
...
mod tests {
    ...
    fn test_xconst_nofma_min_vertical() {
        ...
        let min = unsafe { f64_xconst_avx2_nofma_min_vertical::<512>(&matrix_view) };
    ...
    fn test_xany_nofma_min_vertical() {
        ...
        let min = unsafe { f64_xany_avx2_nofma_min_vertical(&matrix_view) };
        assert_eq!(min, expected_vertical_min);
    }
}
