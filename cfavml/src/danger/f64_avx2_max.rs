...
pub unsafe fn f64_xconst_avx2_nofma_max_vertical<const DIMS: usize>(
    ...
    debug_assert_eq!(output.len(), DIMS, "Output size must be equal to DIMS");
...
/// This method assumes AVX2 instructions are available, if this method is executed
/// on non-AVX2 enabled systems, it will lead to an `ILLEGAL_INSTRUCTION` error.
pub unsafe fn f64_xany_avx2_nofma_max_vertical(
    matrix: &[f64],
    output: &mut [f64]
) {
    let dims = output.len();
    ...
    if offset_from != 0 {
        ...
        while i < (dims - tail) {
            ...
            _mm256_storeu_pd(max_values_ptr.add(i), acc);
        ...
        while i < dims {
            ...
            *output.get_unchecked_mut(i) = max;
...
#[inline(always)]
unsafe fn max_vertical_component(
    i: usize,
...
mod tests {
    ...
    fn test_xconst_nofma_max_vertical() {
        ...
        let mut max = vec![f64::NEG_INFINITY; 512];
        unsafe { f64_xconst_avx2_nofma_max_vertical::<512>(&matrix, &mut max) };
    ...
    fn test_xany_nofma_max_vertical() {
        ...
        let mut max = vec![f64::NEG_INFINITY; 537];
        unsafe { f64_xany_avx2_nofma_max_vertical(&matrix, &mut max) };
        assert_eq!(max, expected_vertical_max);
    }
}
